...
static unsigned bufcount_rd = 250;
...
static struct cdev *cdev_wr;
static struct device *device_rd;
static struct device *device_wr;
static dev_t chardev_num;
...
static irqreturn_t xilinx_dma_irq_handler(int irq, void *data) {
	...
	if (status & XILINX_DMA_DMASR_ERR_IRQ) {
		...
		if (!chan->flush_on_fsync || (errors & ~XILINX_DMA_DMASR_ERR_RECOVER_MASK)) {
			dev_err(chan->dev, "Channel %p has errors %x, cdr %x tdr %x\n", chan, errors, dma_ctrl_read(chan, XILINX_DMA_REG_CURDESC),
	...
	/* interrupt on completion of a transfer in direct register mode */
...
//dev_info(dev, "charrd_read reading size %d\n", buf_size);
	uint32_t copied = 0; //how many bytes have been already copied to the buf
	if (!buf_size) {
		dev_err(dev, "<charrd_read> buf_size is ZERO!!\n");
		return 0;
	}
	uint32_t chunk;/*how much data will be copied*/
	int dma_buf_remain_bytes; /* how many bytes can be copied from the single DMA buffer */
